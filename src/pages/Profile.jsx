...
export const Profile = () => {
  ...
  useEffect(() => {
    document.title = `${username} | Hucschat`
  })
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/friends/requestSent`)
      .then((requests) => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/post/getposts`)
      .then((posts) => {
        setPosts(posts.data);
      })
      .catch((err) => console.log(err));
  ...
  const removePhoto = (id) => {
    axios
      .put(`${process.env.REACT_APP_API}/auth/removephoto/` + id)
      .then((res) => {
        if (res.data === "Success") {
          window.location.reload();
        }
            ...
            {user.username === username ? (
              <div>
                <div className="flex lg:flex-row flex-col">
                  {username === u.username ? (
                      ...
                      {(close) => (
                        <div className="text-center dark:bg-zinc-800 bg-white rounded-xl dark:text-white">
                          ...
                          <button className="w-full p-3 border-b dark:border-gray-600 border-black-50">
                            <label
                              htmlFor="profile"
                              className="px-8 cursor-pointer text-blue-400 font-bold"
                            >
                              Select Photo
                              <input
                                type="file"
                          ...
                          <button
                            onClick={() => {
                              removePhoto(user._id, user.pic);
                              close();
                            }}
                            className="w-full p-3 border-b dark:border-gray-600 border-black-50 text-red-500 font-bold"
                          >
                    ...
                    <>
                      ...
                      {user.pic ? (
                        <Link
                          to={user.status.length ? `/status/${username}` : "#"}
                        >
                          <img
                            src={`${process.env.REACT_APP_API}/profile/${user.pic}`}
                            alt="profile pic"
                            className={`w-20 h-20 ml-4 mt-4 lg:w-40 lg:h-40 lg:ml-64 lg:mt-8 rounded-full border border-black ${
                              user.status.length
                        ...
                        >
                          <img
                            src={profile}
                            alt="profile pic"
                            className={`w-20 h-20 ml-4 mt-4 lg:w-40 lg:h-40 lg:ml-64 lg:mt-8 rounded-full ${
                              user.status.length
                                ? "border-4 border-blue-700 p-1"
                                : "border border-black"
                            } `}
                  ...
                  <div className="text-2xl mt-2 lg:mt-20 lg:ml-20">
                    <span className="font-semibold">
                      {user.posts.length ? (
                        NumberFormat(user.posts.length)
                      ) : (
                        <>0</>
                      )}
                    </span>
                  ...
                  <div className="text-2xl mt-2 lg:mt-20 lg:ml-12">
                    <span className="font-semibold">
                      {user.likes.length ? (
                        NumberFormat(user.likes.length)
                      ) : (
                        <>0</>
                      )}
                    </span>
                    {user.likes.length > 1 ? <> likes</> : <> like</>}
                  </div>
                        ...
                        >
                          {(close) => (
                            <div className="text-center dark:text-white dark:bg-zinc-800 rounded-xl bg-white">
                              ...
                              <button
                                className="rounded-b-xl dark:hover:bg-zinc-700 w-full p-4 "
                                onClick={() => close()}
                              >
                                Cancel
                              </button>
                    ...
                    <>
                      <div className="flex flex-row justify-center gap-4 ml-10 mt-6 lg:ml-10 lg:mt-32">
                        ...
                        {requests.some((r) => r.Friend === username) ? (
                          <button className=" bg-blue-500 text-white dark:bg-gray-800 dark:hover:bg-blue-700 rounded-lg p-2 mb-6 font-semibold transform active:opacity-50">
                            Request sent
                          </button>
                        ) : u.friends.length &&
                          !u.friends.some((f) => f.friend === user.username) ? (
                          <button
                            onClick={(e) =>
                              handleFriend(
                                e,
                ...
                <div className="flex flex-row lg:ml-96 ml-2 ">
                  ...
                  <div className="border-t-2 dark:border-white border-black">
                    POSTS
                  </div>
                  ...
                  <div className="flex flex-wrap mt-10 -ml-16 lg:pb-0 pb-20 lg:-ml-80 gap-1">
                    {user.posts.length ? (
                                ...
                                <div>
                                  ...
                                  p.post.includes(".txt") ? (
                                    <center>
                                      ...
                                      >
                                        <iframe
                                          src={`${process.env.REACT_APP_API}/posts/${p.post}`}
                                          title="file"
                                          className="h-32 w-32 lg:h-40 lg:w-40 border border-black px-6"
                                        ></iframe>
                                      </Link>
                                    </center>
                                  ) : p.post.includes(".mp4") ? (
                      ...
                      <div className="lg:ml-72 ml-20">
                        {username === u.username ? (
                          <div className="text-2xl hover:text-black text-blue-500 font-semibold">
                            <Link to="/createpost">Share your first Post</Link>
                          </div>
                        ) : (
