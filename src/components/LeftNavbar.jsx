  ...
  return (
    <div className=" select-none dark:bg-black dark:text-white bg-white flex lg:flex-col justify-between z-10 fixed w-full bottom-0 lg:top-0 lg:gap-[20px] lg:w-[80px] dark:border-gray-600 border lg:border-r-black">
      <div>
        <Link to="/home" title="home">
          <svg
            ...
            strokeWidth="1.5"
            stroke="currentColor"
            className="h-10 ml-8 lg:mt-4 lg:ml-5 w-8 lg:w-10 hover:scale-110 active:opacity-50"
      ...
      <div className="lg:flex hidden">
        {user ? (
          <Link
            className="flex hover:scale-110  active:opacity-50"
            to="/notification"
            title="notifications"
          >
            <svg
                  ...
                  return (
                    <div>
                      {n.username !== user.username ? (
                        <div className=" fixed left-10 w-6 h-6 text-white bg-red-600 rounded-full">
                          {NumberFormat(user.notifications.length)}
                        </div>
                      ) : null}
                    </div>
                  );
          ...
          {user.pic ? (
            <Link to={`/profile/${user.username}`} title="profile">
              <img
                ...
                alt="profile"
                className="lg:h-10 ml-5 h-8 lg:mt-0 mt-1 w-8 lg:w-10 transform hover:scale-110 active:opacity-50 rounded-full border border-black lg:mr-5 mr-8"
              />
            </Link>
          ) : (
            <Link to={`/profile/${user.username}`} title="profile">
          ...
          trigger={
            <button title="settings">
              <svg
                ...
                strokeWidth="1.5"
                stroke="currentColor"
                className="h-10 ml-5 w-8 lg:w-10 transform hover:scale-110 active:opacity-50"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          {(close) => (
            <div>
              ...
              <button
                ...
                className="dark:bg-zinc-800 dark:text-white bg-white w-full p-4 border-b border-black-50 hover:bg-slate-200 dark:hover:bg-slate-800  active:opacity-50"
              >
                Log Out
              </button>

              <button
                className="dark:bg-zinc-800 dark:text-white bg-white w-full p-4 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-b-lg active:opacity-50"
                onClick={() => close()}
              >
                Cancel
              </button>
